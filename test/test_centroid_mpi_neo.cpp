...
int main(int argc, char** argv) {
    ...
    int subgridX, subgridY, subgridZ;
    assert(argc==8);
    gridX = atoi(argv[1]);
    gridY = atoi(argv[2]);
    gridZ = atoi(argv[3]);
    subgridX = atoi(argv[4]);
    subgridY = atoi(argv[5]);
    subgridZ = atoi(argv[6]);
    std::string inputFilename(argv[7]);

    topology_t topology;
    setupCommunicators(&topology, gridX, gridY, gridZ, subgridX, subgridY, subgridZ);
